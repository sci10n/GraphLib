...
public class HeuristicGraphTest {
	...
	graph.forEachEdge(
		(a, b, e) -> Math.sqrt(Math.pow(Math.abs(a.t1 - b.t1), 2) + (Math.pow(Math.abs(a.t2 - b.t2), 2))));

	System.out.println();
	System.out.println(graph.toString());
	System.out.println("Path");

	for (Vec2 v : GraphUtils.a_star_search(n0, n3, graph,
		((a, b) -> graph.getEdgeValue(a, b) == null ? 0 : graph.getEdgeValue(a, b).intValue()))) {
	    System.out.println(v.toString());
	}

	// Euclidean distance to n3
	graph.forEachEdge(
		(a, b, e) -> Math.sqrt(Math.pow(Math.abs(a.t1 - n3.t1), 2) + (Math.pow(Math.abs(a.t2 - n3.t2), 2))));
	// Notice how the distance between n0 and n1 is the distance from n0 to
	// n3 regardless of no edge between n0 and n3
	System.out.println();
